...
const KelolaRqsBerdaya = ({ onBack }) => {
    ...
    // Verify Modal
    const [verifyModal, setVerifyModal] = useState(null); // the order object

    const loadData = async () => {
        setIsLoading(true);
        const { data: prodData } = await supabase.from('rqs_berdaya').select('*').order('created_at', { ascending: false });
        if (prodData) setProductList(prodData);

        const { data: ordData } = await supabase.from('rqs_orders').select('*').eq('module', 'berdaya').order('created_at', { ascending: false });
    ...
    const handleSave = async () => {
        if (!name.trim() || !seller.trim() || !desc.trim() || (!image && !imagePreview)) return alert("Semua kolom wajib diisi.");
        ...
        if (typeof image !== 'string' && image instanceof File) {
            ...
            const fileName = `berdaya-${Date.now()}.${fileExt}`;
        ...
        const productData = { name, seller, desc, image: finalImageUrl, variants: validVariants.map(v => ({ ...v, price: parseInt(v.price) })) };
        if (editingId) await supabase.from('rqs_berdaya').update(productData).eq('id', editingId);
        else await supabase.from('rqs_berdaya').insert([productData]);
    ...
    const handleDelete = async (id, imageUrl) => {
        if (window.confirm("Yakin hapus produk ini?")) {
            setIsLoading(true);
            await supabase.from('rqs_berdaya').delete().eq('id', id);
    ...
    const openEdit = (item) => {
        ...
        setVariants(item.variants?.length ? item.variants : [{ id: Date.now(), name: 'Default', price: 0 }]);
        setIsModalOpen(true);
    };

    const openAdd = () => { resetForm(); setIsModalOpen(true); };
    const resetForm = () => { setEditingId(null); setName(''); setSeller(''); setDesc(''); setImage(''); setImagePreview(''); setVariants([{ id: Date.now(), name: '', price: '' }]); };
        ...
        <div className="pb-28 animate-in fade-in duration-500 bg-[#FDFBF7] min-h-screen relative">
            {isLoading && (
                <div className="fixed inset-0 bg-white/50 z-[100] flex items-center justify-center backdrop-blur-sm">
                    <div className="bg-white p-4 rounded-xl shadow-lg flex items-center gap-3">
                        <PhosphorIcon icon="circle-notch" className="animate-spin text-blue-600" size={24} />
                    ...
                    <div className="bg-white w-full max-w-md rounded-3xl p-5 relative max-h-[90vh] overflow-y-auto">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="bg-gray-50 p-3 rounded-xl border border-gray-100 text-xs">
                                ...
                                <div className="flex justify-between mt-2 pt-2 border-t border-gray-300 font-bold text-blue-600 text-sm">
                            ...
                            {verifyModal.status === 'Menunggu Verifikasi' && (
                                <div className="flex gap-2 mt-4">
                                    ...
                                    <button disabled={isLoading} onClick={handleConfirmVerification} className="flex-[2] bg-blue-600 text-white font-bold p-3 rounded-xl hover:bg-blue-700 transition">Konfirmasi Pembayaran</button>
                                </div>
                            )}
                        </div>
                    </div>
            ...
            <div className="flex items-center p-4 bg-white sticky top-0 z-10 shadow-sm border-b border-gray-100">
                ...
                <div className="flex-1 text-center pr-10">
                    <h2 className="text-lg font-bold text-[#4A1C14] leading-tight">Kelola Berdaya</h2>
                </div>
            </div>

            <div className="flex bg-white border-b border-gray-100 overflow-x-auto hide-scrollbar">
                <button onClick={() => setActiveTab('katalog')} className={`px-4 py-3 text-xs sm:text-sm font-bold whitespace-nowrap border-b-2 transition-colors ${activeTab === 'katalog' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-400'}`}>Katalog Produk</button>
                <button onClick={() => setActiveTab('verifikasi')} className={`px-4 py-3 text-xs sm:text-sm font-bold whitespace-nowrap border-b-2 transition-colors relative ${activeTab === 'verifikasi' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-400'}`}>
                    Pesanan & Verifikasi
                ...
                <button onClick={() => setActiveTab('proses')} className={`px-4 py-3 text-xs sm:text-sm font-bold whitespace-nowrap border-b-2 transition-colors relative ${activeTab === 'proses' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-400'}`}>
                    Proses Pesanan
                    {processedOrders.length > 0 && <span className="absolute top-2 right-0 w-4 h-4 bg-blue-500 text-white text-[9px] flex items-center justify-center rounded-full">{processedOrders.length}</span>}
                </button>
            </div>
            ...
            <div className="p-4">
                {activeTab === 'katalog' && (
                    <div className="space-y-4">
                        <button onClick={openAdd} className="w-full bg-blue-50 text-blue-600 font-bold p-3 rounded-xl flex items-center justify-center gap-2 border border-blue-100">
                            <PhosphorIcon icon="plus" size={20} weight="bold" /> Tambah Produk
                        </button>
                        {productList.map((p) => (
                            <div key={p.id} className="bg-white rounded-2xl p-4 shadow-sm border border-blue-100 flex gap-3 relative">
                                <div className="absolute top-4 right-4 flex gap-1.5">
                                    <button onClick={() => openEdit(p)} className="p-1.5 text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-lg"><PhosphorIcon icon="pencil-simple" size={14} weight="bold" /></button>
                                ...
                                <div className="w-20 h-20 rounded-xl bg-blue-50 overflow-hidden shrink-0 border border-gray-100">
                                    {p.image ? <img src={p.image} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-blue-500"><PhosphorIcon icon="image" size={32} /></div>}
                                </div>
                                <div className="flex-1 pr-14">
                                    <span className="text-[9px] font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full mb-1 inline-block">{p.seller}</span>
                                    <h4 className="font-bold text-gray-800 text-[13px] leading-tight mb-1">{p.name}</h4>
                                    {p.variants && p.variants.length > 0 && <p className="text-sm font-bold text-blue-600 mb-1">{p.variants.length > 1 ? `${formatRupiah(p.variants[0].price)} - ${formatRupiah(p.variants[p.variants.length-1].price)}` : formatRupiah(p.variants[0].price)}</p>}
                            ...
                            <div key={o.id} className="bg-white rounded-2xl p-4 shadow-sm border border-gray-200 relative">
                                ...
                                <div className="flex justify-between items-end mt-3 border-t border-gray-100 pt-3">
                                    <div>
                                        ...
                                        <p className="text-sm font-bold text-blue-600">{formatRupiah(o.total_amount)}</p>
                                    </div>
                                    <button onClick={() => setVerifyModal(o)} className="bg-blue-50 text-blue-600 text-xs font-bold px-4 py-2 rounded-lg hover:bg-blue-100 transition">
                                        Lihat Detail
                                    </button>
                                </div>
                    ...
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-5 backdrop-blur-sm">
                        <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} className="bg-white w-full max-w-md rounded-3xl overflow-hidden shadow-2xl relative flex flex-col max-h-[75vh] mb-12">
                            <div className="p-4 bg-white border-b border-gray-100 flex items-center justify-between sticky top-0 z-10">
                                <h3 className="text-sm font-bold text-gray-800 flex items-center gap-2"><PhosphorIcon icon={editingId ? "pencil-simple" : "plus-circle"} size={18} className="text-blue-600" />{editingId ? 'Edit Produk' : 'Tambah Produk'}</h3>
                            ...
                            <div className="p-5 overflow-y-auto bg-[#FDFBF7] flex-1">
                                <div className="space-y-4">
                                    ...
                                    <div><label className="text-[10px] font-bold text-gray-600 uppercase block mb-1.5">Nama Penjual</label><input type="text" value={seller} onChange={(e) => setSeller(e.target.value)} className="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none" /></div>
                                    <div>
                                    ...
                                    <div className="bg-blue-50 p-4 rounded-2xl border border-blue-100">
                                        <div className="flex justify-between items-center mb-3">
                                            <label className="text-[10px] font-bold text-blue-800 uppercase">Varian & Harga</label>
                                            <button onClick={handleAddVariant} className="text-[10px] bg-blue-600 text-white px-2 py-1 rounded-lg font-bold flex items-center gap-1"><PhosphorIcon icon="plus" /> Tambah</button>
                                        </div>
                            ...
                            <div className="p-4 bg-white border-t border-gray-100"><button onClick={handleSave} className="w-full bg-blue-600 text-white font-bold py-3.5 rounded-xl shadow-md hover:bg-blue-700 transition-colors">Simpan Produk</button></div>
                        </motion.div>
                    </motion.div>
                )}
            </AnimatePresence>
...
export default KelolaRqsBerdaya;
