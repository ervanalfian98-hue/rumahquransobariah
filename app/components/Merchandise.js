...
const Merchandise = ({ setActiveTab }) => {
    ...
    const loadMyOrders = async () => {
        ...
        const { data } = await supabase.from('rqs_orders').select('*').in('id', savedIds).eq('module', 'merchandise').order('created_at', { ascending: false });
        if (data) setMyOrders(data);
    };

    const loadProducts = async () => {
        const { data, error } = await supabase.from('rqs_merchandise').select('*').order('created_at', { ascending: false });
    ...
    const handleFinishCheckout = async () => {
        ...
        if (proofImage) {
            ...
            const fileName = `bukti-merchandise-${Date.now()}.${fileExt}`;
        ...
        let message = `Halo Admin Merchandise RQS, saya telah melakukan pemesanan:%0A%0A`;
        ...
        <div className="pb-32 animate-in fade-in duration-500 bg-[#FAFAFA] min-h-screen relative">
            <div className="flex items-center p-4 bg-white sticky top-0 z-20 shadow-sm border-b border-gray-100">
                ...
                <div className="flex-1 text-center">
                    <h2 className="text-lg font-bold text-[#4A1C14] leading-tight">Merchandise</h2>
                    <p className="text-[10px] text-amber-600 font-bold">Bangga Bersama RQS</p>
                </div>
                <button onClick={() => { loadMyOrders(); setIsMyOrdersOpen(true); }} className="p-2 ml-2 text-amber-600 hover:bg-amber-50 rounded-xl transition flex flex-col items-center">
                    <PhosphorIcon icon="receipt" size={20} weight="fill" />
                    <span className="text-[8px] font-bold mt-0.5">Pesanan</span>
                </button>
            </div>

            <div className="mx-4 mt-4 bg-gradient-to-br from-amber-500 to-yellow-500 rounded-2xl p-5 text-white shadow-md relative overflow-hidden">
                <div className="relative z-10">
                    <h3 className="text-lg font-bold mb-2 leading-tight">Tampil Keren & Berpahala</h3>
                    <p className="text-xs text-white/90 mb-3 max-w-[90%]">Dapatkan merchandise original RQS. 100% keuntungan digunakan untuk mendukung operasional Rumah Qur'an Sobariah.</p>
                </div>
                <div className="absolute -right-4 -bottom-4 text-white/20 transform -rotate-12"><PhosphorIcon icon="t-shirt" weight="fill" size={120} /></div>
            </div>
            ...
            <div className="px-4 mt-4">
                <div className="bg-white rounded-xl border border-gray-200 flex items-center px-4 py-3 shadow-sm">
                    ...
                    <input type="text" placeholder="Cari merchandise..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full text-sm outline-none bg-transparent"/>
                </div>
            </div>
                    ...
                    {filteredProducts.map(product => {
                        ...
                        return (
                            <div key={product.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden flex flex-col">
                                <div className="h-32 bg-amber-50 relative overflow-hidden border-b border-gray-100">
                                    {product.image ? <img src={product.image} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-amber-300"><PhosphorIcon icon="image" size={32} /></div>}
                                </div>
                                <div className="p-3 flex flex-col flex-1">
                                    ...
                                    <div className="mt-auto">
                                        ...
                                        <p className="text-[13px] font-bold text-amber-600 mb-2">{formatRupiah(selectedVariant.price)}</p>
                                        {qtyInCart > 0 ? (
                                            <div className="flex items-center justify-between bg-amber-50 rounded-lg p-1">
                                                <button onClick={() => removeFromCart(product.id, selectedVariant.id)} className="w-7 h-7 bg-white rounded-md flex items-center justify-center text-amber-600 font-bold">-</button>
                                                <span className="text-xs font-bold text-amber-800">{qtyInCart}</span>
                                                <button onClick={() => addToCart(product.id, selectedVariant.id)} className="w-7 h-7 bg-amber-600 rounded-md flex items-center justify-center text-white font-bold">+</button>
                                            </div>
                                        ) : (
                                            <button onClick={() => addToCart(product.id, selectedVariant.id)} className="w-full bg-white border border-amber-500 text-amber-600 rounded-lg py-1.5 text-[11px] font-bold">Beli Sekarang</button>
                                        )}
                                    </div>
                                </div>
                            </div>
                    ...
                    <div className="bg-white rounded-t-3xl p-5 pb-28 relative z-10 max-h-[90vh] overflow-y-auto animate-in slide-in-from-bottom-full shadow-2xl">
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="font-bold text-lg text-gray-800">
                                {checkoutStep === 'cart' ? 'Keranjang Merchandise' : checkoutStep === 'fill_form' ? 'Info Pengiriman' : checkoutStep === 'select_payment' ? 'Pembayaran' : 'Bukti Transfer'}
                            </h3>
                        ...
                        {checkoutStep === 'cart' && (
                            <>
                                    ...
                                    {Object.entries(cart).map(([cartKey, qty]) => {
                                        ...
                                        return (
                                            <div key={cartKey} className="flex gap-3 bg-white p-2 rounded-xl border border-gray-100 shadow-sm">
                                                <div className="w-16 h-16 bg-amber-50 rounded-lg overflow-hidden shrink-0 border border-gray-100">
                                                    {product.image ? <img src={product.image} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-amber-300"><PhosphorIcon icon="image" size={24}/></div>}
                                                </div>
                                                <div className="flex-1 flex flex-col justify-center">
                                                    ...
                                                    <p className="text-xs text-amber-600 font-bold mt-1">{formatRupiah(variant.price * qty)}</p>
                                                </div>
                                ...
                                <div className="border-t border-gray-100 pt-4">
                                    <div className="flex justify-between items-center mb-4"><span className="text-sm font-bold text-gray-600">Total Belanja</span><span className="text-xl font-bold text-amber-600">{formatRupiah(cartTotal)}</span></div>
                                    <button onClick={() => setCheckoutStep('fill_form')} className="w-full bg-amber-600 text-white rounded-xl py-3.5 font-bold text-sm shadow-md hover:bg-amber-700">Lanjut Pengiriman</button>
                                </div>
                            </>
                        )}
                        {checkoutStep === 'fill_form' && (
                            <div className="space-y-4">
                                ...
                                <button onClick={() => { if(!form.name || !form.phone || !form.address) return alert('Lengkapi data!'); setCheckoutStep('select_payment'); }} className="w-full bg-amber-600 text-white font-bold p-3 rounded-xl">Lanjut Pilih Pembayaran</button>
                        ...
                        {checkoutStep === 'select_payment' && (
                            <div className="space-y-4">
                                ...
                                <button onClick={() => { if(!selectedRekening) return alert("Pilih rekening!"); setCheckoutStep('upload_proof'); }} className="w-full bg-amber-600 text-white font-bold p-3 rounded-xl">Lanjut Upload Bukti</button>
                        ...
                        {checkoutStep === 'upload_proof' && (
                            <div className="space-y-4">
                                <div className="bg-amber-50 p-3 rounded-xl text-center border border-amber-200">
                                    ...
                                    <p className="font-bold text-amber-700 text-lg">{formatRupiah(cartTotal)}</p>
                                </div>
                                <div>
                                ...
                                <button disabled={isLoading} onClick={() => { if(!proofImage) return alert("Upload bukti!"); handleFinishCheckout(); }} className="w-full bg-green-600 text-white font-bold p-3 rounded-xl flex items-center justify-center gap-2">
            ...
            {!isCartOpen && cartCount > 0 && (
                <div className="fixed bottom-24 left-0 right-0 z-40 px-4 flex justify-center animate-in slide-in-from-bottom-4 duration-300">
                    <button onClick={() => setIsCartOpen(true)} className="bg-gray-900 text-white rounded-2xl py-3 px-5 shadow-2xl flex items-center gap-4 w-full max-w-sm border border-gray-800">
                        <div className="relative shrink-0"><PhosphorIcon icon="shopping-cart" weight="fill" size={24} className="text-amber-400" /><span className="absolute -top-2 -right-2 bg-red-500 text-white text-[10px] font-bold w-4 h-4 flex items-center justify-center rounded-full border-2 border-gray-900">{cartCount}</span></div>
                        ...
                        <div className="text-xs font-bold text-gray-900 bg-amber-400 py-1.5 px-3 rounded-xl flex items-center gap-1">Checkout <PhosphorIcon icon="caret-right" weight="bold" /></div>
                    </button>
                </div>
            )}
                                ...
                                myOrders.map(order => (
                                    <div key={order.id} className="bg-white border border-gray-200 rounded-2xl p-4 shadow-sm">
                                        ...
                                        <div className="flex justify-between items-center bg-gray-50 p-2 rounded-lg">
                                            ...
                                            <span className="text-sm font-bold text-amber-600">{formatRupiah(order.total_amount)}</span>
                                        </div>
                                    </div>
                                ))
                            )}
